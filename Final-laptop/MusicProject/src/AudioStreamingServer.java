...
public class AudioStreamingServer {
    ...
    static class ClientHandler implements Runnable {
        ...
        private void processCommand(String command) throws IOException {
            ...
            } else if (command.startsWith("SHARE_PLAYLIST_REQUEST")) {
                String[] parts = command.split(" ", 3);
                if (parts.length >= 3) {
                    String targetUsername = parts[1];
                    String playlistName = parts[2];
                    sharePlaylistRequest(targetUsername, playlistName);
                }
            } else if (command.startsWith("ACCEPT_PLAYLIST")) {
                String[] parts = command.split(" ", 3);
                if (parts.length >= 3) {
                    String senderUsername = parts[1];
                    String playlistName = parts[2];
                    acceptPlaylist(senderUsername, playlistName);
                }
            } else if (command.startsWith("UPDATE_QUEUE")) {
                String queueData = command.substring("UPDATE_QUEUE ".length());
                if (currentRoom != null && currentRoom.getLeader() == this) {
                    currentRoom.updateQueue(queueData);
                }
        ...
        private void sharePlaylistRequest(String targetUsername, String playlistName) {
            ClientHandler targetClient = clients.get(targetUsername);
            if (targetClient != null && currentRoom != null && currentRoom.isMember(targetClient)) {
                if (playlists.containsKey(playlistName)) {
                    targetClient.sendMessage("SHARE_PLAYLIST_REQUEST " + username + " " + playlistName);
                } else {
                    out.println("ERROR Playlist does not exist");
                }
            } else {
                out.println("ERROR User not available to share playlist");
            }
        }

        private void acceptPlaylist(String senderUsername, String playlistName) {
            ClientHandler senderClient = clients.get(senderUsername);
            if (senderClient != null) {
                List<String> tracks = senderClient.playlists.get(playlistName);
                if (tracks != null) {
                    playlists.put(playlistName, new ArrayList<>(tracks));
                    savePlaylist(playlistName);
                    out.println("SHARE_PLAYLIST " + playlistName + " " + String.join(",", tracks));
                } else {
                    out.println("ERROR Playlist does not exist on sender's side");
                }
            } else {
                out.println("ERROR Sender not available");
            }
        }
    ...
    static class Room {
        ...
        public void playTrack(String trackName) {
            if (currentTrack != null && !currentTrack.equals(trackName)) {
        ...
        public void pauseTrack() {
            isPaused = true;
            broadcastMessage("PAUSE", null);
        }

        public void resumeTrack() {
            isPaused = false;
            broadcastMessage("RESUME", null);
        }
        ...
        public void updateQueue(String queueData) {
            songQueue.clear();
    ...
    static class AudioStreamingHandler implements Runnable {
        ...
        public void run() {
            try {
                ...
                // Read the username, track name, and start frame
                String username = dataIn.readUTF();
                String trackName = dataIn.readUTF();
                int startFrame = dataIn.readInt();
